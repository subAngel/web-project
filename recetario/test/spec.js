...
describe("logger", function () {
	...
	it("should log when on response.success", function () {
		nock("http://swag.com").get("/").reply(200, "{}");

		return axios.get("http://swag.com").then(function () {
			expect(getArgThenRestore(console.log, 0)).to.contain("Success");
		});
	});

	it("should log when on response.error", function () {
		nock("http://swag.com").get("/").reply(400, "{}");

		return axios.get("http://swag.com").catch(function () {
